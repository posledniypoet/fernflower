...
public class ClassWriter {
      ...
      if (!clInit && !dInit) {
        ...
        for (int i = methodWrapper.varproc.getFirstParameterPosition(); i < md.params.length; i++) {
          if (mask == null || mask.get(i) == null) {
            ...
            Type paramType;
            if (descriptor != null) paramType = descriptor.parameterTypes.get(paramCount);
            else paramType = md.params[i];
        ...
        if (root != null && !methodWrapper.decompiledWithErrors) { // check for existence
          try {
            ...
            hideMethod = code.length() == 0 &&
              ...
              isSyntheticRecordMethod(cl, mt, code) || (isRecordDefaultConstructor(node, mt) || isRecordDefaultGetter(node, mt) && DecompilerContext.getOption(IFernflowerPreferences.HIDE_RECORD_DEFAULT));
  ...
  private boolean isRecordDefaultConstructor(ClassNode node, StructMethod mt) {
    return (mt.getName().equals("<init>") && node.classStruct.superClass.value.equals("java/lang/Record"));
  }

  private boolean isRecordDefaultGetter(ClassNode node, StructMethod mt) {
    boolean checkFields = false;
    for (StructField field : node.classStruct.getFields()) {
      if (field.getName().equals(mt.getName())) {
        checkFields = true;
        break;
      }
    }
    return (node.classStruct.superClass.value.equals("java/lang/Record") && mt.getLocalVariables() == 1 && checkFields);
  }
